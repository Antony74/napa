...
NapaPkg.prototype.install = function (done) {
  ...
  function cacheInstall () {
    if (typeof self._mock === 'function') {
      ...
      var extraction = extract(self.installTo, {finish: cb})

      fs.createReadStream(self.cacheTo).pipe(extraction)

      extraction.once('error', function (error) {
        self.log.info('Unable to read from cache (' + self.installTo + '): ' + error.message)

        self.log.info('Try again without cache')
        self.useCache = false
        self.install(done)

        // ignore subsequent errors (would be preferable to terminate this extraction operation)
        extraction.on('error', function () {})
      })
    }
  }
